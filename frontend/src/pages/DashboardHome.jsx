...
const DashboardHome = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const statsRes = await axios.get("http://localhost:8880/api/analytics/stats");
        setStats(statsRes.data);
        const recentRes = await axios.get("http://localhost:8880/api/analytics/recent-activity");
          ...
          <div className="space-y-6">
            {stats.industry_distribution.length > 0 ? stats.industry_distribution.map((ind, i) => (
                ...
                <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-600" style={{ width: `${(ind.value / stats.total_leads) * 100}%` }} />
                </div>
                </div>
